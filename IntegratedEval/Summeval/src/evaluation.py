...
import pandas as pd
from src.utils import _inject, openai_call, claude_call, split_json_objects, parse, wavg
from src.config import MAX_TOKENS, PRESET

BACKENDS_ALL = {"OpenAI": openai_call, "Claude": claude_call}

last_summary = {}
last_eval_result = {}

def evaluate(article, summary, variant, active_back, temp,
             w_cov, w_align, w_hall, w_rel, w_bias, show_ev, prompts=None):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=6) as exe:
        ...
        for fut in concurrent.futures.as_completed(futures):
            ...
            if not show_ev and "hallucination" in parsed:
                claims = parsed["hallucination"].get("claims_checked", [])
